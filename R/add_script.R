add_script <- function(name, script_folder = TRUE){
  script_temp <- "##########################################################
# Name of file
...
### END OF SCRIPT ###"
  ...
  if(script_folder){
    writeLines(script_temp, con = file.path("3. Scripts", name))
    file.edit(file.path("3. Scripts", name))
  } else {
    writeLines(script_temp, con = paste0(name))
    file.edit(file.path(name))
  }
}
